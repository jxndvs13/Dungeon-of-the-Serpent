...
class Player:
    ...
    def AttackSkip(self):
        ...
        if self.Weapon == "Cleave":
            cleaveable = []
            for c in cords:
                if c == f_cord(P.x-1,P.y) or c == f_cord(P.x-1,P.y-1) or c == f_cord(P.x,P.y-1) or c == f_cord(P.x+1,P.y-1) or c == f_cord(P.x+1,P.y) or c == f_cord(P.x+1,P.y+1) or c == f_cord(P.x,P.y+1) or c == f_cord(P.x-1,P.y+1):
                    cleaveable.append(c)
            for c in cleaveable:
                if c.CordType != " ":
                    f_enemy(c.x,c.y).Hurt(self.Dmg)
        Attacks -= 1
        RunTurn()
class Weapon:
    def __init__(self):
